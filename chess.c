...
int chess_valid(char *board, int *move, int turn) {
  ...
  if (board[move[0]] == 'p') {
    if (board[move[1]] == ' ') {
      if (move[1] == move[0] + 8) {
        return 1;
      } else if (move[0] / 8 == 1 && move[1] == move[0] + 16) {
        return 1;
      }
    }
    if (chess_turn(board[move[0]] + 7) == CHESS_WHITE || chess_turn(board[move[0]] + 9 == CHESS_WHITE)) {
      if (move[1] == move[0] + 7 || move[1] == move[0] + 9) {
        return 1;
      }
    }
...
int **chess_get_moves(char *board, int turn) {
  ...
  // them with chess_valid() before. It's NULL terminated
